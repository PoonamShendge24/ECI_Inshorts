...
import {View, StyleSheet, StatusBar, Platform, Text,AsyncStorage,TouchableOpacity,NativeModules,FlatList} from 'react-native';
...
class AdminScreen extends Component {
  ...
  state = {
	myScore:0,
	data : [],
	numberArray: [],dateArray: [],serverData: [],redeemArray: []
  };
...
fetch("https://auth.reloadly.com/oauth/token", requestOptions)
  ...
  //this.setState({data: jsonString});
   // console.log("balance is "+jsonString);
    console.log("top up called"+index);
    this.addData(index);
  var request = new XMLHttpRequest();
...
//         }).done();
  ...
  //5this.setState({ serverData: [] });
  	var refData = firebase.database().ref("TopUpData");
		refData.on("child_added",snap=>{
		 console.log("data is "+snap.child("Contact").val());
		 this.state.numberArray.push(snap.child("Contact").val());
		 this.state.dateArray.push(snap.child("Email").val());
		 this.state.redeemArray.push(snap.child("Redeem").val());
		 if(snap.child("Redeem").val()<1)
		 	this.state.serverData.push("Phone number "+snap.child("Contact").val()+" has requested 25 INR");
...
//   }
  ...
  addData(index){
     ...
     firebase.database().ref('TopUpData/Data' + (index+1)).update({
		Redeem: 1,
	 })
	this.setState(this.state);
  }
...
handleBackOnPress
  render() {
    ...
    console.log('admin render called'+this.state.serverData.length);
   
      return (
        <View style={styles.container}>
        <View style={{flexDirection:"row",paddingTop:30}}>
	    <Text style={{fontSize: 25,color:'black',alignItems:'center',marginLeft:20,fontFamily: 'Roboto-Bold'}}>Notifications</Text>
		...
		<FlatList
              ...
              renderItem = {({ item , index}) => 
                  <View style = { styles.item }>
                      ...
                      <TouchableOpacity style={styles.but} onPress={()=>this.topUp(index)}>
